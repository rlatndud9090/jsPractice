...
const isPrimeNumber = (number) => {
  ...
  if (number <= 3) return true; // 2와 3은 소수임
  if (number % 2 === 0 || number % 3 === 0) return false; // 2 또는 3으로 나눠지면 소수가 아님

  // 소수가 아닌 수는 √number보다 작은 약수를 가짐
...
let resultArr = [];
for (let i = 1; i < inputVal / 2; i++) {
  if (isPrimeNumber(i) && isPrimeNumber(inputVal - i)) {
    resultArr.push(i.toString() + " + " + (inputVal - i).toString());
  }
}
